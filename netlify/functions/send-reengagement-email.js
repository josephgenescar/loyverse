...
const campaignId = process.env.REENGAGEMENT_CAMPAIGN_ID || 'return-to-konektem-2026-09';
...
exports.handler = async function(event) {
  ...
  try {
    const users = await supabaseRequest(`/rest/v1/konektem_users?select=email,bizname&email=not.is.null&order=created_at.asc&offset=${offset}&limit=${limit}`, 'GET');
    const uniqueUsers = Array.from(new Map((users || []).filter(user => user.email).map(user => [user.email.toLowerCase(), user])).values());
    if (dryRun) return json(200, { dryRun: true, campaignId, count: uniqueUsers.length, recipients: uniqueUsers.map(user => user.email) });

    const results = { sent: [], skipped: [], failed: [] };
    for (const user of uniqueUsers) {
      const email = user.email.toLowerCase();
      const existing = await supabaseRequest(`/rest/v1/email_campaign_sends?campaign_id=eq.${encodeURIComponent(campaignId)}&email=eq.${encodeURIComponent(email)}&status=eq.sent&select=id&limit=1`, 'GET');
      ...
      try {
        ...
        await supabaseRequest('/rest/v1/email_campaign_sends', 'POST', {
          campaign_id: campaignId, email, status: 'sent', provider_id: provider.id || null
        }, { Prefer: 'resolution=ignore-duplicates' });
        results.sent.push(email);
      } catch (error) {
        results.failed.push({ email, error: error.message });
        await supabaseRequest('/rest/v1/email_campaign_sends', 'POST', {
          campaign_id: campaignId, email, status: 'failed', error_message: error.message
        }, { Prefer: 'resolution=ignore-duplicates' }).catch(() => {});
      }
    }
    return json(200, Object.assign({ dryRun: false, campaignId }, results));
  } catch (error) {
